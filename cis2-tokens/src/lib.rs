//! Upgradable tokean_A smart contract (Concordium's canonical 
//! implementation following the CIS-2 standard)
#![cfg_attr(not(feature = "std"), no_std)]
use concordium_cis2::{Cis2Event, *};
use concordium_std::{collections::BTreeMap, *};

/// The id of the token A token in this contract.
const TOKEN_ID_A: ContractTokenId = TokenIdU64(1u64);
...
impl Serial for TokenAEvent {
    fn serial<W: Write>(&self, out: &mut W) -> Result<(), W::Err> {
        match self {
            TokenAEvent::NewAdmin(event) => {
                out.write_u8(NEW_ADMIN_EVENT_TAG)?;
                event.serial(out)
            }
            TokenAEvent::Cis2Event(event) => event.serial(out),
        }
    }
}

/// Manual implementation of the `TokenAEventSchema` which includes both the
...
impl schema::SchemaType for TokenAEvent {
...
impl<S: HasStateApi> State<S> {
    ...
    fn balance(
        ...
        ensure_eq!(token_id, &TOKEN_ID_A, ContractError::InvalidTokenId);
...
fn contract_init<S: HasStateApi>(
    ...
    logger.log(&TokenAEvent::Cis2Event(Cis2Event::Mint(MintEvent {
        token_id: TOKEN_ID_A,
    ...
    logger.log(&TokenAEvent::Cis2Event(Cis2Event::TokenMetadata::<_, ContractTokenAmount>(
        TokenMetadataEvent {
            token_id: TOKEN_ID_A,
            metadata_url,
        },
    ...
    logger.log(&TokenAEvent::NewAdmin(NewAdminEvent {
        new_admin: invoker,
    }))?;
...
/// get amount of tokens A and transfer the tokens if the sender
/// is not the receiver.
#[receive(
    contract = "cis2_token_a",
    name = "get_token_a",
    parameter = "GetTokenAParams",
    error = "ContractError",
    enable_logger,
    mutable,
)]
fn contract_get_token_a<S: HasStateApi>(
    ...
    // Parse the parameter.
    let params: GetTokenAParams = ctx.parameter_cursor().get()?;
    ...
    // Update the state.
    state.mint(&TOKEN_ID_A, params.amount.micro_ccd.into(), &receive_address, state_builder)?;

    // Log the newly minted tokens.
    logger.log(&TokenAEvent::Cis2Event(Cis2Event::Mint(MintEvent {
        token_id: TOKEN_ID_A,
        amount:   ContractTokenAmount::from(params.amount.micro_ccd),
        owner:    sender,
    })))?;
    ...
    if sender != receive_address {
        logger.log(&TokenAEvent::Cis2Event(Cis2Event::Transfer(TransferEvent {
            token_id: TOKEN_ID_A,
            amount:   ContractTokenAmount::from(params.amount.micro_ccd),
            from:     sender,
            to:       receive_address,
...
/// state-mutative functions (get_token_a, transfer, updateOperator) cannot be
...
fn contract_update_operator<S: HasStateApi>(
    ...
    for param in params {
        ...
        logger.log(&TokenAEvent::Cis2Event(
...
mod tests {
    ...
    // The metadata url for the token A token.
    const INITIAL_TOKEN_METADATA_URL: &str = "https://some.example/token/token_a";
    ...
    fn initial_state<S: HasStateApi>(state_builder: &mut StateBuilder<S>) -> State<S> {
        ...
        state
            .mint(&TOKEN_ID_A, 400u64.into(), &ADDRESS_0, state_builder)
    ...
    fn test_init() {
        ...
        let balance0 =
            state.balance(&TOKEN_ID_A, &ADDRESS_0).expect_report("Token is expected to exist");
        claim_eq!(
            balance0,
            0u64.into(),
        ...
        claim!(
            logger.logs.contains(&to_bytes(&TokenAEvent::Cis2Event(Cis2Event::Mint(MintEvent {
                owner:    ADDRESS_0,
                token_id: TOKEN_ID_A,
        ...
        claim!(
            logger.logs.contains(&to_bytes(&TokenAEvent::Cis2Event(Cis2Event::TokenMetadata::<
                _,
                ContractTokenAmount,
            >(
                TokenMetadataEvent {
        ...
        claim!(
            logger.logs.contains(&to_bytes(&TokenAEvent::NewAdmin(NewAdminEvent {
                new_admin: ADDRESS_0,
            }))),
    ...
    fn test_set_metadata_url() {
        ...
        let new_url = "https://some.example/token/token_a/updated".to_string();
    ...
    fn test_transfer_account() {
        ...
        state
            .mint(&TOKEN_ID_A, 400.into(), &ADDRESS_0, &mut state_builder)
            .expect_report("Failed to setup state");
        let mut host = TestHost::new(state, state_builder);
        ...
        let balance0 = host
            .state()
            .balance(&TOKEN_ID_A, &ADDRESS_0)
            .expect_report("Token is expected to exist");
        let balance1 = host
            .state()
            .balance(&TOKEN_ID_A, &ADDRESS_1)
            .expect_report("Token is expected to exist");
        claim_eq!(
            balance0,
            300.into(),
        ...
        claim_eq!(
            logger.logs[0],
            to_bytes(&TokenAEvent::Cis2Event(Cis2Event::Transfer(TransferEvent {
                from:     ADDRESS_0,
                to:       ADDRESS_1,
                token_id: TOKEN_ID_A,
                amount:   ContractTokenAmount::from(100),
    ...
    fn test_add_operator() {
        ...
        claim_eq!(
            logger.logs[0],
            to_bytes(&TokenAEvent::Cis2Event(
                Cis2Event::<ContractTokenId, ContractTokenAmount>::UpdateOperator(
                    UpdateOperatorEvent {
                        owner:    ADDRESS_0,
                        operator: ADDRESS_1,
    ...
    fn test_get_token_a() {
        ...
        // Set up the parameter.
        let get_params = GetTokenAParams {
            to:   Receiver::from_account(ACCOUNT_1),
            amount: Amount::from_micro_ccd(amount),
            data: AdditionalData::empty(),
        };
        let parameter_bytes = to_bytes(&get_params);
        ctx.set_parameter(&parameter_bytes);

        let result: ContractResult<()> =
            contract_get_token_a(&ctx, &mut host, &mut logger);

        // Check the result.
        claim!(result.is_ok(), "Results in rejection");

        // Check the token A balance of ADDRESS_1.
        let balance0 = host
            .state()
            .balance(&TOKEN_ID_A, &ADDRESS_1)
            .expect_report("Token is expected to exist");
        claim_eq!(
            ...
            "ADDRESS_1 should have received tokens"
        );
    ...
    fn test_no_execution_of_state_mutative_functions_when_paused() {
        ...
        claim_eq!(
            result,
            Err(ContractError::Custom(CustomContractError::ContractPaused)),
            "Get Token A should fail because contract is paused"
        );
    }
}